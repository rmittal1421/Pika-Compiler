...
public class ArrayDeallocCodeGenerator implements SimpleCodeGenerator {
	...
	public ASMCodeFragment generate(ParseNode node) {
		...
		Macros.readIOffset(frag, Record.RECORD_STATUS_OFFSET);        // [arrPointer status]
		Macros.storeITo(frag, RunTime.ARRAY_STATUS_FLAGS);            // [arrPointer]
		frag.add(Duplicate);                                          // [arrPointer arrPointer]
		Macros.readIOffset(frag, Record.ARRAY_SUBTYPE_SIZE_OFFSET);   // [arrPointer subtypeSize]
		Macros.storeITo(frag, RunTime.ARRAY_SUBTYPE_SIZE);            // [arrPointer]
		frag.add(Duplicate);                                          // [arrPointer arrPointer]
		Macros.readIOffset(frag, Record.ARRAY_LENGTH_OFFSET);         // [arrPointer length]
		...
		frag.add(PushI, Record.STATUS_FLAG_FOR_REFERENCE);
		...
		Macros.loadIFrom(frag, RunTime.ARRAY_STATUS_FLAGS);           // [baseForCurrentElement status]
		frag.add(PushI, Record.STATUS_FLAG_FOR_DELETE_AND_PERM);      // [baseForCurrentElement 0b0011]
		frag.add(BTOr);                                               // [base 1 or _]
		frag.add(PushI, Record.BINARY_F);                             // [base 1/_ 1]
...
//		Macros.loadIFrom(frag, RunTime.ARRAY_INDEXING_ARRAY);
		...
		Macros.loadIFrom(frag, RunTime.ARRAY_STATUS_FLAGS);
		frag.add(PushI, Record.TURN_DELETE_BIT_1);
		frag.add(BTOr);
		Macros.writeIPtrOffset(frag, RunTime.ARRAY_INDEXING_ARRAY, Record.ARRAY_STATUS_FLAGS_OFFSET);
